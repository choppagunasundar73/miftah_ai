...
export function MemoryContextPanel({
    ...
    return (
        <div className={`fixed left-0 top-16 h-[calc(100vh-4rem)] bg-[#957D65] text-[#E3DCD4] transition-transform duration-300 z-30 shadow-2xl shadow-[#222635]/20 ${isOpen ? 'translate-x-0' : '-translate-x-full'
            } w-80 overflow-hidden backdrop-blur-sm`}>

            {/* Header */}
            <div className="p-6 border-b border-[#E3DCD4]/20">
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center space-x-3">
                        <div className="w-10 h-10 bg-[#E3DCD4]/20 rounded-xl flex items-center justify-center backdrop-blur-sm">
                            <User size={20} className="text-[#E3DCD4]" />
                        </div>
                        <div>
                            <h2 className="text-xl font-serif font-medium tracking-wide">Memory</h2>
                            <p className="text-xs text-[#E3DCD4]/70 tracking-wider">NAVIGATION</p>
                        </div>
                    </div>
                    <button
                        onClick={onToggle}
                        className="p-2 hover:bg-[#E3DCD4]/10 rounded-lg transition-all duration-200 hover:scale-105 lg:hidden"
                    >
                        <ChevronLeft size={18} />
                    </button>
                </div>
            ...
            <div className="flex-1 overflow-y-auto">
                <div className="p-6 space-y-6">
                    {/* Active Section Info */}
                    <div className="bg-[#E3DCD4]/10 backdrop-blur-sm rounded-xl p-4 border border-[#E3DCD4]/20">
                        <div className="flex items-center space-x-3 mb-2">
                            {(() => {
                                const activeItem = navigationSections.find(s => s.id === activeSection);
                                const Icon = activeItem?.icon || MessageCircle;
                                return <Icon size={20} className="text-[#E3DCD4]" />;
                            })()}
                            <h3 className="font-serif font-medium text-lg tracking-wide">
                    ...
                    <div>
                        ...
                        <div className="space-y-3">
                            {recentActivity.map((activity, i) => (
                                <div key={i} className="bg-[#E3DCD4]/5 backdrop-blur-sm rounded-lg p-3 border border-[#E3DCD4]/10">
                                    <div className="flex items-start justify-between">
                                        <div className="flex-1">
                                            <p className="text-sm font-medium text-[#E3DCD4] mb-1">{activity.action}</p>
                                            <div className="flex items-center space-x-2 text-xs text-[#E3DCD4]/60">
                                                <Clock size={10} />
                                                <span>{activity.time}</span>
                                            </div>
                                        </div>
                                        <div className={`w-2 h-2 rounded-full mt-1 ${activity.type === 'booking' ? 'bg-[#E3DCD4]' :
                                            activity.type === 'saved' ? 'bg-[#E3DCD4]/70' : 'bg-[#E3DCD4]/40'
                                            }`}></div>
                                    </div>
                                </div>
                            ))}
